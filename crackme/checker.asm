; -----------------------------------------------------------------------------
; | FnCheckHash
; | Args:   si - byte sequence address
; |         cx - byte sequence length
; |         ax - target hash
; | Assumes:    ds = cs, VerificationJumpSegment is set to cs
; | Destroys:   ax, bx, cx, si, es registers
; | Returns:    ax - check result, sets VerificationJumpOffset and in case of success
; -----------------------------------------------------------------------------
FnCheckHash proc
    push ax
    ...
    pop bx                  ; compute hash and move target hash to bx

    cmp ax, bx              ; compare hash
    jne @@WrongHash

    mov ax, 1
    mov word ptr VerificationJumpOffset, offset VerificationJumpEnd

    ret
@@WrongHash:
    xor ax, ax
    mov word ptr VerificationJumpOffset, offset FnDestroyVideoBuffers
    ret
endp

; -----------------------------------------------------------------------------
; | FnComputeHash (Uses CRC-16-ANSI)
; | Args:   si - byte sequence address
; |         cx - byte sequence length
; | Assumes:    ds = cs
; | Destroys:   bx,cx, si, es registers
; | Returns:    ax - hash value
; -----------------------------------------------------------------------------
FnComputeHash proc
    push cs
    pop  ax

    mov bx, 0ffffh                      ; bx - hash value
@@HashLoop:                             
    mov ax, 0                           
    lodsb                               
    mov es, ax                          
    ...
    mov di, ax                          
    add di, offset CrcTable             ; di = ax + &CrcTable
                                        
    mov dx, [di]                        ; dx = CrcTable [ax]
                                        
    mov ax, bx                          
    shr ax, 8                           ; ax = crc >> 8
                                        
    xor ax, dx                          
    mov bx, ax                          ; crc = (crc >> 8) ^ CrcTable [al ^ bl]
